...
class DAB_branch1(nn.Module):
    def __init__(self, conv, n_feat, kernel_size, reduction):
        super(DAB_branch1, self).__init__()
...
class DAG_branch1(nn.Module):
    def __init__(self, conv, n_feat, kernel_size, reduction, n_blocks):
        super(DAG_branch1, self).__init__()
        self.n_blocks = n_blocks
        modules_body = [
            DAB_branch1(conv, n_feat, kernel_size, reduction) \
...
class DASR_branch1(nn.Module):
    def __init__(self, args, conv=common.default_conv):
        super(DASR_branch1, self).__init__()
        ...
        # body
        modules_body = [
            DAG_branch1(common.default_conv, n_feats, kernel_size, reduction, n_blocks) \
    ...
    def forward(self, x, k_v_kernel, k_v_noise):
        ...
        return x, res
#####################################################
class DAB_branch2(nn.Module):
    def __init__(self, conv, n_feat, kernel_size, reduction):
        super(DAB_branch2, self).__init__()
...
class DAG_branch2(nn.Module):
    def __init__(self, conv, n_feat, kernel_size, reduction, n_blocks):
        super(DAG_branch2, self).__init__()
        self.n_blocks = n_blocks
        modules_body = [
            DAB_branch1(conv, n_feat, kernel_size, reduction) \
            for _ in range(n_blocks)
    ...
    def forward(self, x):
        ...
        return res
class DASR_branch2(nn.Module):
    def __init__(self, args, conv=common.default_conv):
        super(DASR_branch2, self).__init__()
        ...
        # body
        modules_body = [
            DAG_branch2(common.default_conv, n_feats, kernel_size, reduction, n_blocks) \
            for _ in range(self.n_groups)
        ]
        modules_body.append(conv(n_feats, n_feats, kernel_size))
        self.body = nn.Sequential(*modules_body)
...
'''class Fusion_model(nn.Module):
    ...
    def forward(self, x):
        ...
        mask = self.decoder(x_enc)
        return mask
        
class Restore_model(nn.Module):
    def __init__(self):
...
class Restore_model(nn.Module):
    def __init__(self):
        ...
        self.head = nn.Conv2d(6, 6, kernel_size=3, padding=1)
...
class BlindSR(nn.Module):
    def __init__(self, args):
        ...
        # Generator
        self.G_1 = DASR_branch1(args)#.cuda()
        self.G_2 = DASR_branch2(args)#.cuda()
        # Encoder
        self.E_kernel = MoCo(base_encoder=Encoder_2_regressor)#.cuda()
        self.E_noise = MoCo(base_encoder=Encoder_2_regressor)
        #self.fusion_net = Fusion_model()
        self.restore_net = Restore_model()
    def forward(self, x):
        if self.training:
            x_kernel = x[0]
            x_noise = x[1]
            ...
            sr1,_ = self.G_1(x_query_kernel, fea_kernel, fea_noise)
            sr2,_ = self.G_2(x_query_kernel, fea_kernel, fea_noise)
            sr_cat = torch.cat((sr1,sr2),1)
            #mask1= self.fusion_net(sr1,sr2)
            #mask2 = 1-mask1
            #sr_fuse =sr1*mask1 +sr2*mask2
            
            sr_final = self.restore_net(sr_cat)
            return sr1,sr2,sr_final, logits_kernel, logits_noise, labels
        else:
            x_kernel= x[1]
            x_noise = x[1]
            x_img = x[0]
            ...
            sr,_ = self.G_2(x_img, fea_kernel, fea_noise)
