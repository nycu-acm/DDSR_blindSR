...
## here the encoder is for kernel predictor iterative
## different with Encoder: input channel:3->6(because we have LR & SR)
class Encoder_2(nn.Module):
    def __init__(self):
        super(Encoder_2, self).__init__()
        #stride= enlarge scale
        self.SR_encode = nn.Conv2d(3, 32, kernel_size=3, padding=1,stride=4)
        self.LR_encode = nn.Conv2d(3, 32, kernel_size=3, padding=1) 
        self.E = nn.Sequential(
    ...
    def forward(self, x):
        x_LR = x[0]
        x_SR = x[1]
        LR_encoded = self.LR_encode(x_LR)
        SR_encoded = self.SR_encode(x_SR)
        x = torch.cat((LR_encoded,SR_encoded),dim=1)
        fea = self.E(x).squeeze(-1).squeeze(-1)
        out = self.mlp(fea)
        return fea, out
...
class BlindSR(nn.Module):
    def __init__(self, args):
        ...
        self.model_deblur_iter = deblur_DASR(args)
        self.model_denoise = denoising_net(args)
        # Encoder
        self.E_kernel = MoCo(base_encoder=Encoder)
        self.E_kernel_iter = MoCo(base_encoder=Encoder_2)
    ...
    def forward(self, x):
        if self.training:
            ...
            fea_kernel_i0, logits_kernel, labels_kernel = self.E_kernel(lr_denoise_1, lr_denoise_2)
            sr_1_i0 = self.model_deblur(lr_denoise_1,fea_kernel_i0)
            sr_2_i0 = self.model_deblur(lr_denoise_2,fea_kernel_i0)
            ## iterative 4 times
            fea_kernel_i1, logits_kernel_i1, labels_kernel_i1 = self.E_kernel_iter([lr_denoise_1,sr_1_i0],[lr_denoise_2,sr_2_i0])
            sr_1_i1 = self.model_deblur(lr_denoise_1,fea_kernel_i0)
            sr_2_i1 = self.model_deblur(lr_denoise_2,fea_kernel_i0)
            fea_kernel_i2, logits_kernel_2, labels_kernel_i2 = self.E_kernel_iter([lr_denoise_1,sr_1_i1],[lr_denoise_2,sr_2_i1])
            sr_1_i2 = self.model_deblur(lr_denoise_1,fea_kernel_i1)
            sr_2_i2 = self.model_deblur(lr_denoise_2,fea_kernel_i1)
            fea_kernel_i3, logits_kernel_i3, labels_kernel_i3 = self.E_kernel_iter([lr_denoise_1,sr_1_i2],[lr_denoise_2,sr_2_i2])
            sr_1_i3 = self.model_deblur(lr_denoise_1,fea_kernel_i2)
            sr_2_i3 = self.model_deblur(lr_denoise_2,fea_kernel_i2)
            fea_kernel_i4, logits_kernel_i4, labels_kernel_i4 = self.E_kernel_iter([lr_denoise_1,sr_1_i3],[lr_denoise_2,sr_2_i3])
            sr_1_i4 = self.model_deblur(lr_denoise_1,fea_kernel_i3)
            # sr_2_i4 = model_deblur(lr_denoise_2,fea_kernel_i3)                      
            #return sr, logits_noise, labels_noise, logits_kernel, labels_kernel, lr_denoise_1, lr_denoise_2
            return sr_1_i4, logits_kernel, labels_kernel, logits_kernel_i4, labels_kernel_i4
        else:
            fea_noise = self.E_noise(x, x)
            lr_denoise_1= self.model_denoise(x,fea_noise)
            fea_kernel_i0 = self.E_kernel(lr_denoise_1, lr_denoise_1)
            sr_1_i0 = self.model_deblur(lr_denoise_1,fea_kernel_i0)
            ## iterative 4 times
            fea_kernel_i1 = self.E_kernel_iter([lr_denoise_1,sr_1_i0],[lr_denoise_1,sr_1_i0])
            sr_1_i1 = self.model_deblur(lr_denoise_1,fea_kernel_i0)
            fea_kernel_i2 = self.E_kernel_iter([lr_denoise_1,sr_1_i1],[lr_denoise_1,sr_1_i1])
            sr_1_i2 = self.model_deblur(lr_denoise_1,fea_kernel_i1)
            fea_kernel_i3 = self.E_kernel_iter([lr_denoise_1,sr_1_i1],[lr_denoise_1,sr_1_i2])
            sr_1_i3 = self.model_deblur(lr_denoise_1,fea_kernel_i2)
            fea_kernel_i4 = self.E_kernel_iter([lr_denoise_1,sr_1_i3],[lr_denoise_1,sr_1_i3])
            sr_1_i4 = self.model_deblur(lr_denoise_1,fea_kernel_i3)
            return sr_1_i0,sr_1_i4,lr_denoise_1