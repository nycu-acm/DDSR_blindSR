...
class Encoder_2(nn.Module):
    ...
    def forward(self, x):
        ...
        #qk = qk/10000 ##1/13 experiment
        sorted, indices = torch.sort(qk, descending=True)
        y=torch.zeros(sorted.shape).cuda()
        qk_new = torch.ones(sorted.shape).cuda()
        for i in range(sorted.shape[0]):
          #print('before',qk[i])
          #print(sorted[i,5].item())          
          qk_new[i] = torch.where(qk[i]>sorted[i,10].item(),qk[i],y[i])
          #print('after',qk_new[i])
        qk = F.softmax(qk_new, dim=1)
