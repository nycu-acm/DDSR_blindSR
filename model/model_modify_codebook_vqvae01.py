...
class DASR(nn.Module):
    ...
    def forward(self, x, k_v_kernel, k_v_noise):
        ...
        return x, kernel_codebook_loss, noise_codebook_loss, kernel_indices, noise_indices, kernel_quant, noise_quant,k_v_kernel,k_v_noise
'''#codebook(VQVAE v2)
class DASR(nn.Module):
    def __init__(self, args, conv=common.default_conv):
        super(DASR, self).__init__()
...
class BlindSR(nn.Module):
    ...
    def forward(self, x):
        if self.training:
            ...
            # degradation-aware SR            
            sr, kernel_codebook_loss, noise_codebook_loss, kernel_indices, noise_indices, kernel_quant, noise_quant,k_v_kernel,k_v_noise = self.G(x_img, fea_kernel, fea_noise)

            return sr, kernel_codebook_loss, noise_codebook_loss, kernel_indices, noise_indices, kernel_quant, noise_quant,k_v_kernel,k_v_noise
