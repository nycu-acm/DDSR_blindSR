...
class DASR(nn.Module):
    def __init__(self, args, conv=common.default_conv):
        ...
        # new
        # base representation
        self.kernel_embed = nn.Embedding(1, 64)
        #self.noise_embed = nn.Embedding(1, 64)
        #self.kernel_embed.requires_grad = False         
               
    def forward(self, x, k_v_kernel, k_v_noise):
        b,c = k_v_kernel.shape
        kernel_fea_ = self.kernel_embed.weight.repeat(b,1)
        k_v_kernel = torch.cat((kernel_fea_,k_v_kernel),1)
        k_v_kernel = self.compress_kernel(k_v_kernel)
        # v1
        '''noise_fea_ = self.noise_embed.weight.repeat(b,1)
        k_v_noise = torch.cat((noise_fea_,k_v_noise),1)
...
class BlindSR(nn.Module):
    ...
    def forward(self, x):
        if self.training:
            ...
            #print(torch.equal(self.kernel_embed.weight,self.noise_embed.weight))
            #kernel_fea_ = self.kernel_embed.weight.repeat(32,1)
            #noise_fea_ = self.noise_embed.weight.repeat(32,1)
            ...
            #kernel_fea = torch.cat((kernel_fea_,fea_kernel),1)
            #noise_fea = torch.cat((noise_fea_,fea_noise),1)
            sr = self.G(x_img, kernel_fea, noise_fea)
            return sr
        else:
            x_img = x[0]
            x_freq = x[1]
            # degradation-aware represenetion learning
            fea_kernel = self.E_kernel(x_freq, x_freq)
            fea_noise = self.E_noise(x_freq, x_freq)
            #kernel_fea = self.kernel_embed.weight.repeat(1,1)
            #noise_fea = self.noise_embed.weight.repeat(1,1)            
            # degradation-aware SR
