...
class FuseBLK(nn.Module):
    def __init__(self, n_feat, kernel_size, reduction):
        ...
        self.da_conv1 = nn.Conv2d(n_feat*2, n_feat, kernel_size, stride=1, padding=(kernel_size//2))
    ...
    def forward(self, x):
        ...
        :param x[1]: degradation representation: B * C
        '''
        #param x[1]: kernel representation: B * C
        #param x[2]: noise representation: B * C
...
class SFT_Layer(nn.Module):
    ...
    def forward(self, feature_maps, para_maps_kernel,para_maps_noise):
        para_maps_kernel = self.fc_k(para_maps_kernel)
        para_maps_noise = self.fc_k(para_maps_noise)
        x = self.dab([feature_maps, para_maps_kernel,para_maps_noise])
        return x
...
class RRDB_SFT(nn.Module):
    ...
    def forward(self, input):
        out = self.SFT(input[0], input[1],input[2])
        out = self.RDB1(out)
        out = self.RDB2(out)
        out = self.RDB3(out)
        return [out * 0.2 + input[0], input[1],input[2]]
...
class CDSR(nn.Module):
    ...
    def forward(self, x, fea_kernel,fea_noise):
        ...
        fea_map_kernel = self.compress_kernel(fea_kernel)
        fea_map_noise = self.compress_noise(fea_noise)
        lr_fea = self.conv_first(x)
        fea = self.RRDB_trunk([lr_fea, fea_map_kernel,fea_map_noise])
...
class BlindSR(nn.Module):
    def __init__(self, args):
        ...
        # Encoder
        self.E_kernel = MoCo(base_encoder=Encoder_2_regressor)
    ...
    def forward(self, x):
        ...
        # x = x / 255.0
        if self.training:
            x_k_query = x[1][:, 0, ...]  # b, c, h, w
            x_k_key = x[1][:, 1, ...]  # b, c, h, w
            x_n_query = x[2][:, 0, ...]  # b, c, h, w
            x_n_key = x[2][:, 1, ...]  # b, c, h, w
            x_q = x[0]    
            # degradation-aware represenetion learning
            fea_k, logits_k, labels_k = self.E_kernel(x_k_query, x_k_key)
            fea_n, logits_n, labels_n = self.E_noise(x_n_query, x_n_key)
            # degradation-aware SR
            sr = self.G(x_query, fea_k,fea_n)
            sr = sr * 255.0
            return sr, logits_k, labels_k,logits_n, labels_n
        else:
            x_img = x[0]
            x_wavlet = x[1]

            # degradation-aware represenetion learning
            fea_k = self.E_kernel(x_wavlet, x_wavlet)
            fea_n = self.E_noise(x_wavlet, x_wavlet)
            #print(type(fea_k))
            #print(fea_n.shape)
            # degradation-aware SR
            sr = self.G(x_img, fea_k,fea_n)
            #print(fea_n.shape)
            sr = sr * 255.0
...
if __name__ == '__main__':
    ...
    flops, params = get_model_complexity_info(model, (3, 256, 256), as_strings=True,
                                              print_per_layer_stat=True)  # 不用?batch_size大小，默?batch_size=1
    print('Flops:  ' + flops)
    print('Params: ' + params)
